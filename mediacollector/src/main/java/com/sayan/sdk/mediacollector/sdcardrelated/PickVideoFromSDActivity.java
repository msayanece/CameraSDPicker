...
public class PickVideoFromSDActivity extends Activity {
     ...
     * Open Gallery
     */
    private void videoFileChooserIntent() {
        Intent intent = new Intent();
        intent.setType("video/*");
        intent.setAction(Intent.ACTION_GET_CONTENT);
        intent.putExtra(Intent.EXTRA_LOCAL_ONLY, true);     //only local storage image allowed
        startActivityForResult(Intent.createChooser(intent, "Select video"), SELECT_VIDEO_INTENT);
    }
    ...
    private void onSelectFromGalleryResult(Intent data) {
        if (data != null) {
            ...
            if (selectFile != null) {
                ...
                SDCardProvider.VideoPickerListener videoPickerListener = SDCardProvider.getInstance().getVideoPickerListener();
                if (file == null) {
                    file = getFile(this, selectFile);
                }
                if (videoPickerListener != null) {
                    videoPickerListener.onVideoPicked(file);
                }
            } else {
